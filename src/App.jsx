...
\
const beneficios = [\
  \{ id:"alimentacion", nombre:"Alimentaci\'f3n",        emoji:"\uc0\u55356 \u57213 \u65039 ", gravado:false, topeUVT:41  \},\
  \{ id:"movilidad",    nombre:"Movilidad",            emoji:"\uc0\u55357 \u56983 ", gravado:false, topeUVT:null\},\
  \{ id:"dotacion",     nombre:"Dotaci\'f3n",             emoji:"\uc0\u55357 \u56404 ", gravado:false, topeUVT:null\},\
  \{ id:"educacion",    nombre:"Educaci\'f3n",            emoji:"\uc0\u55357 \u56538 ", gravado:false, topeUVT:null\},\
  \{ id:"viaticos",     nombre:"Vi\'e1ticos Accidentales",emoji:"\uc0\u9992 \u65039 ", gravado:false, topeUVT:null\},\
  \{ id:"guarderias",   nombre:"Guarder\'edas",           emoji:"\uc0\u55357 \u56438 ", gravado:false, topeUVT:null\},\
  \{ id:"salud",        nombre:"Salud y Bienestar",    emoji:"\uc0\u55356 \u57317 ", gravado:true,  topeUVT:null\},\
  \{ id:"remoto",       nombre:"Trabajo Remoto",       emoji:"\uc0\u55357 \u56507 ", gravado:true,  topeUVT:null\},\
  \{ id:"rewards",      nombre:"Rewards",              emoji:"\uc0\u55356 \u57217 ", gravado:true,  topeUVT:null\},\
  \{ id:"mascotas",     nombre:"Mascotas",             emoji:"\uc0\u55357 \u56382 ", gravado:true,  topeUVT:null\},\
  \{ id:"entretenimiento",nombre:"Entretenimiento",   emoji:"\uc0\u55356 \u57260 ", gravado:true,  topeUVT:null\},\
];\
\
const TASAS = \{ segSocial:0.12522, vacaciones:0.04170, cesantias:0.09330, prima:0.08330, parafiscales:0.04000 \};\
const TASA_TOTAL = Object.values(TASAS).reduce((a,b)=>a+b,0);\
const fmt = n => "$"+Math.round(n).toLocaleString("es-CO");\
const fmtPct = n => (n*100).toFixed(1)+"%";\
\
const Tag = (\{color, bg, children\}) => (\
  <span style=\{\{fontSize:10, fontWeight:500, padding:"2px 7px", borderRadius:20, background:bg, color, letterSpacing:"0.03em"\}\}>\{children\}</span>\
);\
\
...
\
  const cardStyle = \{\
    background: GLIM_CARD,\
    border: `1px solid $\{GLIM_BORDER\}`,\
    borderRadius: 16,\
...
\
  const labelStyle = \{\
    fontSize: 11, fontWeight: 500, color: GLIM_MUTED,\
    textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 6, display:"block",\
  \};\
\
  const inputStyle = \{\
    width:"100%", boxSizing:"border-box",\
    background:"rgba(255,255,255,0.05)", border:`1px solid $\{GLIM_BORDER\}`,\
    borderRadius:8, color:GLIM_TEXT, padding:"8px 12px", fontSize:14,\
...
\
      ...
      <div style=\{\{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1.5rem" \}\}>\
        <div>\
          ...
          <p style=\{\{ margin:"2px 0 0", fontSize:12, color:GLIM_MUTED \}\}>UVT 2026: $52.374 \'b7 Res. DIAN 000238/2025</p>\
        </div>\
        \{/* Pacto toggle */\}\
        <div style=\{\{ textAlign:"right" \}\}>\
          ...
          <div style=\{\{ display:"flex", alignItems:"center", gap:8, justifyContent:"flex-end" \}\}>\
            ...
            <div onClick=\{()=>setPacto(p=>!p)\} style=\{\{\
              width:44, height:24, borderRadius:12, cursor:"pointer",\
              background: pacto ? GLIM_GREEN : "rgba(255,255,255,0.12)",\
              position:"relative", transition:"background 0.2s", flexShrink:0,\
            \}\}>\
              <div style=\{\{\
                position:"absolute", top:3, left: pacto ? 23 : 3,\
                width:18, height:18, borderRadius:"50%",\
                background: pacto ? GLIM_DARK : "white",\
                transition:"left 0.2s",\
              \}\}/>\
            </div>\
          </div>\
          \{pacto && <span style=\{\{ fontSize:11, color:GLIM_GREEN, display:"block", marginTop:4 \}\}>Art. 29-1 ET, P\'e1rr. 1\'b0</span>\}\
        </div>\
      </div>\
\
      \{/* Par\'e1metros */\}\
      <div style=\{cardStyle\}>\
        <span style=\{labelStyle\}>Par\'e1metros base</span>\
        <div style=\{\{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"12px 16px" \}\}>\
          <div>\
            <label style=\{labelStyle\}>Salario bruto / empleado</label>\
            <input type="number" value=\{salario\} step="100000"\
              onChange=\{e=>setSalario(Math.max(0,Number(e.target.value)))\}\
              style=\{inputStyle\} />\
          </div>\
          <div>\
            <label style=\{labelStyle\}>N\'famero de empleados</label>\
            <input type="number" value=\{empleados\} min="1" step="1"\
              onChange=\{e=>setEmpleados(Math.max(1,Number(e.target.value)))\}\
              style=\{inputStyle\} />\
          </div>\
          <div>\
            <label style=\{labelStyle\}>Tarifa retenci\'f3n empleado: <span style=\{\{color:GLIM_GREEN\}\}>\{retencion\}%</span></label>\
            <input type="range" min="0" max="39" step="1" value=\{retencion\}\
              onChange=\{e=>setRetencion(Number(e.target.value))\}\
              style=\{\{ width:"100%", accentColor:GLIM_GREEN, marginTop:6 \}\} />\
          </div>\
        </div>\
      </div>\
\
      ...
      <div style=\{cardStyle\}>\
        <div style=\{\{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1rem" \}\}>\
          ...
          <span style=\{\{\
            ...
            background: excede40 ? "rgba(239,68,68,0.15)" : "rgba(0,195,122,0.12)",\
            color: excede40 ? DANGER_COLOR : GLIM_GREEN,\
          \}\}>\
            \{fmtPct(pct40)\} del salario \{excede40 ? "\'97 supera el 40%" : "\'97 dentro del 40%"\}\
          </span>\
        </div>\
\
        <div style=\{\{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 \}\}>\
          \{beneficios.map(b => \{\
            ...
            return (\
              <div key=\{b.id\} style=\{\{\
                background: GLIM_CARD2,\
                border: `1px solid $\{monto>0 ? (esFavorable ? "rgba(0,195,122,0.3)" : esGravado ? "rgba(245,158,11,0.3)" : "rgba(0,195,122,0.25)") : GLIM_BORDER\}`,\
              ...
              \}\}>\
                ...
                <div style=\{\{ display:"flex", alignItems:"center", gap:4 \}\}>\
                  ...
                  <input type="number" value=\{monto\} min="0" step="50000"\
        ...
        <table style=\{\{ width:"100%", borderCollapse:"collapse", fontSize:13 \}\}>\
          ...
          <tbody>\
            \{[\
              ...
              \{ l:"Cesant\'edas e intereses (9,330%)", t:TASAS.cesantias \},\
              ...
              return (\
                <tr key=\{i\}>\
                  ...
                  <td style=\{\{ textAlign:"right", padding:"9px 0", color: diff>0 ? GLIM_GREEN : "rgba(255,255,255,0.3)", fontWeight: diff>0?500:400, borderBottom:`1px solid $\{GLIM_BORDER\}` \}\}>\{diff>0?fmt(diff):"\'97"\}</td>\
                </tr>\
              );\
            \})\}\
            <tr>\
            ...
            \{pacto && ahorroRet>0 && (\
              <tr>\
                <td style=\{\{ padding:"8px 0", color:GLIM_GREEN, fontSize:12 \}\}>Retenci\'f3n ahorrada (pacto colectivo)</td>\
                <td style=\{\{ textAlign:"right", padding:"8px 0", color:GLIM_MUTED, fontSize:12 \}\}>\'97</td>\
                <td style=\{\{ textAlign:"right", padding:"8px 0", color:GLIM_MUTED, fontSize:12 \}\}>\'97</td>\
...
\
      ...
      <div style=\{\{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:"1rem" \}\}>\
        ...
        <div style=\{\{\
          background: ahorroAnio>0 ? "rgba(0,195,122,0.1)" : GLIM_CARD,\
          border: `1px solid $\{ahorroAnio>0 ? "rgba(0,195,122,0.3)" : GLIM_BORDER\}`,\
        ...
        \}\}>\
          ...
          <p style=\{\{ margin:"3px 0 0", fontSize:11, color: ahorroAnio>0?GLIM_GREEN:GLIM_MUTED \}\}>\{empleados\} empleado\{empleados!==1?"s":""\} \'d7 12 meses</p>\
        </div>\
      </div>\
\
      \{excede40 && (\
        <div style=\{\{ background:"rgba(239,68,68,0.08)", border:`1px solid rgba(239,68,68,0.3)`, borderRadius:12, padding:"10px 16px", marginBottom:"1rem" \}\}>\
          <p style=\{\{ margin:0, fontSize:13, color:DANGER_COLOR \}\}>\
            Los beneficios configurados (\{fmtPct(pct40)\}) superan el l\'edmite del 40% del salario \'97 Art. 30, Ley 1393/2010. Ajusta los montos para cumplir la normativa.\
          </p>\
        </div>\
      )\}\
\
      <p style=\{\{ fontSize:11, color:"rgba(255,255,255,0.2)", textAlign:"center", margin:0 \}\}>\
        Estimaci\'f3n orientativa \'b7 Brick Abogados oct. 2025 \'b7 UVT 2026: $52.374 \'b7 SMMLV 2026: $2.000.000\
      </p>\
    </div>\
  );\
\}}